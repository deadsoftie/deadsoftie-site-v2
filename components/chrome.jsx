    ...
    <header className="chrome">
      <div className="brand">
        <div style={{ display: "flex", gap: 0 }}>
          ...
          <div className="brand-mark accent-2" style={{ marginLeft: -4, transform: "translateY(2px)" }}>s</div>
        </div>
  ...
  <footer className="foot">
    <div>
      <div style={{ marginBottom: 8 }}>
        <span style={{ color: "var(--fg-dim)" }}>deadsoftie</span> · maintained since 2019 · last build {new Date().toLocaleDateString("en-CA")}
      </div>
...
const Placeholder = ({ w = "100%", h = 160, label = "figure" }) => (
  <div className="placeholder" style={{ width: w, height: h }}>{label}</div>
);
