(function () {
  ...
  function render() {
    ...
    if (activeTag !== 'all') {
      data = data.filter(function (e) { return e.tags && e.tags.includes(activeTag); });
    }
    ...
    if (activeSort === 'long')  data.sort(function (a, b) { return b.words - a.words; });
    if (activeSort === 'short') data.sort(function (a, b) { return a.words - b.words; });
    if (activeSort === 'new')   data.sort(function (a, b) { return b.date.localeCompare(a.date); });

    countEl.textContent = data.length + ' matching';

    // Group by year
    const grouped = {};
    data.forEach(function (e) { (grouped[e.year] = grouped[e.year] || []).push(e); });
    ...
    listEl.innerHTML = years.map(function (year) {
      ...
      return '<div class="year-group">' +
        '<div class="year-header">' +
          ...
          '<span class="year-count">' + rows.length + ' ' + pluralize(rows.length, 'essay', 'essays') + '</span>' +
        '</div>' +
        rows.map(function (e) {
          const mins = calcReadTime(e.words, e.read_time);
          const dateStr = e.date ? e.date.replace(/(\d{4})-(\d{2})-(\d{2})/, function (_, y, m, d) {
          ...
          const pinTag = e.pinned
            ...
            : '';
          return '<a href="' + e.url + '" class="post-row"' + pinStyle + '>' +
            '<div class="date">' + dateStr + '</div>' +
            '<div class="post-title">' + pinTag + e.title + '</div>' +
            '<div class="post-tags">' + tags + '</div>' +
            '<div class="read">' + mins + ' min</div>' +
          '</a>';
        }).join('') +
      '</div>';
    }).join('');
  ...
  tagBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      tagBtns.forEach(function (b) { b.classList.toggle('is-active', b === btn); });
      render();
    });
  });
