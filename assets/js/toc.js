(function () {
  ...
  const totalMin = parseInt(progressMeta && progressMeta.textContent.match(/\d+ min$/)?.[0]) || 1;
  ...
  function onScroll() {
    ...
    if (progressMeta) {
      ...
      progressMeta.textContent = pct + '% · ' + minRead + ' of ' + totalMin + ' min';
    }
  ...
  if (sidenotes && fnList) {
    ...
    document.querySelector('.footnotes') && document.querySelector('.footnotes').remove();
  }
